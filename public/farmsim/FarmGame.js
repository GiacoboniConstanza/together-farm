...
/// Simulation tick length (ms between `updateInternal` runs).
FarmGame.prototype.simFrameMs = 400;
/// One in-game calendar day lasts this many real-time seconds (3600 = 1 hour).
FarmGame.prototype.realSecondsPerGameDay = 3600;

Object.defineProperty(FarmGame.prototype, 'daysPerFrame', {
	get: function () {
		return this.simFrameMs / (this.realSecondsPerGameDay * 1000);
	},
	enumerable: true,
});
...
FarmGame.prototype.init = function(){
	if(window.__TOGETHER_EMBED__){
		this.deserialize(window.__TOGETHER_SAVE__ || null);
		return;
	}
...
FarmGame.prototype.update = function(deltaTime){
	...
	// Repeat the frame procedure in constant interval (cap avoids freezing after long background tabs).
	var maxSteps = 240;
	while(frameTime < this.time && 0 < maxSteps--){
		this.updateInternal();
		this.time -= frameTime;
	}
}
